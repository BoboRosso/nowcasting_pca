...
class Pybbg():
    def __init__(self, host='localhost', port=8194):
        ...
        if not self.session.start():
            ...
            raise ImportError("Failed to start bloomberg session")
    ...
    def bdh(self, ticker_list,
        ...
        if override:
            ...
            for k in override:
                ...
                override1.setElement('value', override[k])
        ...
        while (True):
            ...
            for msg in ev:
                ...
                for fd in fieldData:
                    for j in fd.elements():
                        if j.name() == "date":
                            continue
                        data[(ticker, str(j.name()))][fd["date"]] = j.getValue()
    ...
    def bdhOverrideEPS(self, ticker_list, fld_list, override, start_date, end_date=date.today().strftime('%Y%m%d'),
                       periodselection='DAILY', currency=None, nonTradingDayFillOption='NON_TRADING_WEEKDAYS'):
        """
        Get ticker_list and field_list
        return pandas multi level columns dataframe
        """
        ...
        if currency:
            request.set("currency", currency)
        request.getElement("overrides")
        overrides = request.getElement("overrides")
        override1 = overrides.appendElement()
        override1.setElement("fieldId", override[0])
        override1.setElement("value", override[1])
        # print("Sending Request:", request)
        # Send the request
        self.session.sendRequest(request)
        # defaultdict - later convert to pandas
    ...
    # def bdp_with_ID(self, ticker, identifier, fld_list):
    ...
    #     for t in ticker:
    #         securities.appendValue("/isin/912828GM6@BGN"))
